...
def close_room(request, room_id):
    ...
    if request.user == room.creator:
        ...
        if overlaping:
            ...
            categories = ','.join([category for category in categoey_freq.items()])
        ...
        if room.latitude and room.longitude:
            url = f"https://api.yelp.com/v3/businesses/search?latitude={room.latitude}&longitude={room.longitude}&term=food&categories={categories}&price={prices}&open_now=true&attributes={attributes}&sort_by=best_match&limit=12"
        else:
            url = f"https://api.yelp.com/v3/businesses/search?location={room.city}&term=food&categories={categories}&price={prices}&open_now=true&attributes={attributes}&sort_by=best_match&limit=12"
